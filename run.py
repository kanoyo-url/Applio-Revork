...
if True == True:
    ...
    try:
        ...
        logger.warn(
            f"Something is listening on port {port}; check open connection and restart Kano."
        )
...
def cli_infer(com):
    ...
    print("Kanoyo-VC Infer-CLI: Starting the inference...")
    vc_data = vc.get_vc(model_name, protection_amnt, protect1)
    print(vc_data)
    print("Kanoyo-VC Infer-CLI: Performing inference...")
    ...
    if "Success." in conversion_data[0]:
        print(
            "Kanoyo-VC Infer-CLI: Inference succeeded. Writing to %s/%s..."
        ...
        print(
            "Kanoyo-VC Infer-CLI: Finished! Saved output to %s/%s"
            % ("assets", "audios", "audio-outputs", output_file_name)
        )
    else:
        print("Kanoyo-VC Infer-CLI: Inference failed. Here's the traceback: ")
...
def cli_extract_feature(com):
    ...
    print("Kanoyo-VC Feature Extraction: Starting...")
    ...
    execute_generator_function(generator)
    print("Kanoyo-VC Feature Extraction: Finished")
...
def cli_train(com):
    ...
    print("Kanoyo-VC Train-CLI: Training...")
    click_train(
        model_name,
        sample_rate,
        has_pitch_guidance,
...
def cli_train_feature(com):
    ...
    model_name = com[0]
    version = com[1]
    print("Kanoyo-VC Train Feature Index-CLI: Training... Please wait")
    generator = train_index(model_name, version)
    execute_generator_function(generator)
    print("Kanoyo-VC Train Feature Index-CLI: Done!")
...
def cli_extract_model(com):
    ...
    if extract_small_model_process == "Success.":
        print("Kanoyo-VC Extract Small Model: Success!")
    else:
        print(str(extract_small_model_process))
        print("Kanoyo-VC Extract Small Model: Failed!")
